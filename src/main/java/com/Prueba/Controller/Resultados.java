...
public class Resultados extends HttpServlet {
    ...
    public Resultados() {
     ...
     private String extraerFecha(String fecha) {
    	...
    	String formateada= LocalDate.parse(fecha.substring(0,fecha.indexOf("T")),DateTimeFormatter.ofPattern("yyyy-MM-dd")).
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String opcion=request.getParameter("selector");
		  ...
		  try {
			  ...
			  System.out.println(resp.statusCode()); String cuerpoJson=resp.body();
			  
			  
			  JSONObject objeto= new
			  JSONObject(cuerpoJson).getJSONArray("response").getJSONObject(0);
			  
			  fecha=objeto.getJSONObject("fixture").get("date").toString(); JSONObject
			  equipos= objeto.getJSONObject("teams");
			  local=equipos.getJSONObject("home").get("name").toString();
			  visitante=equipos.getJSONObject("away").get("name").toString(); JSONObject
			  subobjetoPuntajes = objeto.getJSONObject("goals");
